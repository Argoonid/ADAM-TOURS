    ...
    >
      <div>
        ...
        <div className="relative h-56 overflow-hidden bg-slate-100">
          ...
          {/* Бейдж популярно */}
          {tour.featured && (
            <div className="absolute top-4 left-4 bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 text-xs font-black px-3 py-1.5 rounded-full shadow-md flex items-center gap-1 z-10">
          ...
          {/* Счетчик мест */}
          <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md text-slate-900 text-xs font-bold px-3 py-1.5 rounded-full shadow-md flex items-center gap-1.5 border border-slate-100 z-10">
            <Users className="w-3.5 h-3.5 text-amber-600" />
            <span>{t('tour.seats', 'Мест')}: {tour.availableSeats}</span>
          </div>

          {/* Длительность и локация поверх градиента */}
